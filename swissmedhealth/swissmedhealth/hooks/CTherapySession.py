...
class CTherapySession(OriginalTherapySession):
    ...
    def validate_duplicate(self):
        ...
        # Initialize an empty list to store practitioner names
        practitioner_list = []

        # Loop through custom_practitioner field to collect practitioner names
        for practitioner in self.custom_practitioner:
            practitioner_list.append(practitioner.name1)
        # pl = practitioner_list
        # Calculate the end time of the current session
        end_time = datetime.datetime.combine(
            getdate(self.start_date), get_time(self.start_time)
        ) + datetime.timedelta(minutes=flt(self.duration))

        start_time_str = self.start_time  # Assuming start_time is defined
        end_time_str = end_time

        # Prepare the SQL query to check for overlapping sessions
        end_time_str = end_time.time().strftime("%H:%M:%S")
        query = """SELECT ts.name as name FROM `tabTherapy Session` ts
            JOIN `tabPractitioner` pr ON ts.name = pr.parent
            WHERE ts.start_date = %s AND ts.name != %s AND ts.docstatus != 2 AND pr.name1 in %s 
            AND (ts.start_time >= %s AND ts.start_time <= %s )
        """

        # Execute the SQL query
        if practitioner_list:
            practitioner_overlaps = frappe.db.sql(query, (self.start_date, self.name, tuple(practitioner_list), start_time_str, end_time_str)
            )

            if practitioner_overlaps:
                frappe.throw(" Please choose another time or practitioner.")
