...
def get_available_units(doctype, txt, searchfield, start, page_len, filters):
    ...
    unit1 = frappe.db.sql(
        """select service_unit,start_time,duration from `tabTherapy Session` where start_date = '{0}' and
                 service_unit is not null and workflow_state in ("Scheduled","Arrived") and
                name != '{1}' and service_unit like %(txt)s """.format(
            date_str, filters["name"]
        ),
        {"txt": "%{0}%".format(txt)},
        as_dict=1,
    )
    ...
    for dict in unit1:
        ...
        if (str(time_str) >= str(start_time) and str(time_str) < str(end_time)) or (
            str(end_time_str) > str(start_time) and str(end_time_str) <= str(end_time)
        ):
            res_unit.append(dict["service_unit"])

    unit2 = frappe.db.sql(
        """select service_unit from `tabTherapy Type Service Units` where parenttype = "Therapy Type" 
                  and parent = "{0}" and service_unit like %(txt)s """.format(
    ...
    result = list(set(unit2) - set(res_unit))
...
def get_practitioner(doctype, txt, searchfield, start, page_len, filters):
    ...
    practitioner = frappe.db.sql(
        """select p.practitioner as practitioner, h.practitioner_name as practitioner_name from `tabPractitioner Multiselect` as p join `tabHealthcare Practitioner` as h on p.practitioner = h.name where p.parenttype = "Therapy Type" 
                  and p.parent = "{0}" and h.practitioner_name like %(txt)s """.format(
            filters["therapy_type"]
        ),
        {"txt": "%{0}%".format(txt)},
        as_dict=1,
